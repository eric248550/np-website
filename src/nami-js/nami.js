...
class MultipleWalletApi {
  ...
  async getAddress() {
    ...
    const addressHex = Buffer.from(
      (await enableWallet.getUsedAddresses())[0],
      "hex"
    );
  ...
  async getNetworkId() {
    ...
    let networkId = await enableWallet.getNetworkId()
    return {
      id: networkId,
  ...
  async getBalance() {
    ...
    const valueCBOR = await enableWallet.getBalance();
    // console.log(valueCBOR);
    const value = this.S.Value.from_bytes(Buffer.from(valueCBOR, "hex"));

    const utxos = await enableWallet.getUtxos();
  ...
  async getUserDelegation(networkId){
    const rewardAddress = await this.getRewardAddress_bech32();
    const stake = await this._blockfrostRequest({
  ...
  async delegationTransaction(networkId, targetPoolId) {
    const delegation = await this.getUserDelegation(networkId);
    ...
    const stakeCredential = this.S.RewardAddress.from_address(
      this.S.Address.from_bytes(Buffer.from(rewardAddress, "hex"))
    ).payment_cred()

    let utxos = await this.getUtxosHex();
    ...
    // fake output (no need coinselection in here) ---->
    ...
    const certificates = this.S.Certificates.new();
    if (!delegation.active) {
      certificates.add(
        this.S.Certificate.new_stake_registration(
            this.S.StakeRegistration.new(stakeCredential)
        )
      );
    }
    ...
    certificates.add(
        this.S.Certificate.new_stake_delegation(
          this.S.StakeDelegation.new(
            stakeCredential,
            this.S.Ed25519KeyHash.from_bech32(targetPoolId)
        )
        )
    );
  ...
  async getRewardAddress_bech32(){
    return await this.S.Address.from_bytes(
      Buffer.from( await this.getRewardAddress(), "hex")
    ).to_bech32();
  }
  async getRewardAddress() {
    const enableWallet = await this.Wallet.enable();
    const rewardAddress = await enableWallet.getRewardAddresses();

    return rewardAddress[0];
  }
  ...
  async getUtxosHex() {
    const enableWallet = await this.Wallet.enable();
    return await enableWallet.getUtxos()
  }
  ...
  async _getProtocolParameter(networkId) {
    ...
    return {
      linearFee: {
        ...
        // add 123 fee to fix fee calculation error
        minFeeB: (p.min_fee_b + 50000).toString(),
      },
...
export default MultipleWalletApi;
